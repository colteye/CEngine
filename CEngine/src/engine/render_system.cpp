...
void RenderSystem::RegisterMesh(Mesh *mesh)
{
	...
	for (auto& it : mesh_data_dict)
	{
		...
		material_render_dict[it.first].counts.push_back(static_cast<GLsizei>(vertex_size / sizeof(glm::vec3)));
		material_render_dict[it.first].start_indexes.push_back(static_cast<GLint>(current_buffers->vertex_buffer_length / sizeof(glm::vec3)));
