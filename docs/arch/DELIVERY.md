...
| ozz-animation | use behind `IAnimationBackend` | runtime skeleton/clip conversion, sampling, two-pose blending, and local-to-model evaluation stay private to the Ozz backend; CEngine assets remain backend-neutral and CEngine owns playback policy, events, handles, budgets, and GPU skinning |
