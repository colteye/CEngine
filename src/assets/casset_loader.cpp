...
bool ReadHeader(ByteView bytes, DiskCAssetHeader &value)
{
    ...
    return ReadU16LE(bytes, offset, value.version) && ReadU16LE(bytes, offset, value.header_size) &&
           ReadU32LE(bytes, offset, value.composition_type) && ReadU32LE(bytes, offset, value.object_count) &&
           ReadU32LE(bytes, offset, value.object_table_offset) && ReadU32LE(bytes, offset, value.component_count) &&
           ...
           ReadU32LE(bytes, offset, value.source_path_offset) && ReadU32LE(bytes, offset, value.source_path_size) &&
...
bool ReadObjectRow(ByteView bytes, std::size_t offset, DiskCAssetObject &value)
{
    if (!ReadU32LE(bytes, offset, value.name_offset) || !ReadU32LE(bytes, offset, value.name_size) ||
        !ReadU32LE(bytes, offset, value.role) || !ReadU32LE(bytes, offset, value.object_type) ||
        !ReadU32LE(bytes, offset, value.parent_name_offset) || !ReadU32LE(bytes, offset, value.parent_name_size) ||
        !ReadU32LE(bytes, offset, value.first_component) || !ReadU32LE(bytes, offset, value.component_count))
    {
        return false;
    }
...
bool CAsset::Parse()
{
    ...
    if (header_.magic != CAssetPayloadMagic || header_.version != CAssetPayloadVersion ||
        header_.header_size != sizeof(DiskCAssetHeader))
    {
        return AssetError("casset payload header is not supported");
    }

    const std::size_t object_table_size = static_cast<std::size_t>(header_.object_count) * sizeof(DiskCAssetObject);
    const std::size_t component_table_size =
        static_cast<std::size_t>(header_.component_count) * sizeof(DiskCAssetComponent);
    if (header_.object_table_offset > payload.size || object_table_size > payload.size - header_.object_table_offset ||
        header_.component_table_offset > payload.size ||
        component_table_size > payload.size - header_.component_table_offset ||
    ...
    object_table_ = {payload.data + header_.object_table_offset, object_table_size};
    component_table_ = {payload.data + header_.component_table_offset, component_table_size};
    string_table_ = {payload.data + header_.string_table_offset, header_.string_table_size};

    std::string_view view;
    if (!StringViewAt(header_.source_path_offset, header_.source_path_size, view) ||
        !StringViewAt(header_.collection_name_offset, header_.collection_name_size, view))
    {
        return AssetError("casset source or collection name is outside the string table");
    }

    for (std::uint32_t index = 0; index < header_.object_count; ++index)
    {
        CAssetObject object;
        ...
        if (object.first_component > header_.component_count ||
            object.component_count > header_.component_count - object.first_component)
        {
            return AssetError("casset object component range is invalid");
        }
    }

    for (std::uint32_t index = 0; index < header_.component_count; ++index)
    {
...
{
    std::string_view view;
    StringViewAt(header_.source_path_offset, header_.source_path_size, view);
    return view;
}
...
bool CAsset::Object(std::uint32_t index, CAssetObject &object) const
{
    ...
    if (!ReadObjectRow(object_table_, static_cast<std::size_t>(index) * sizeof(DiskCAssetObject), disk_object))
    {
        return false;
    }
    ...
    for (std::size_t component = 0; component < object.world_from_local.size(); ++component)
    {
        object.world_from_local[component] = disk_object.world_from_local[component];
    }
    return true;
}
...
bool CAsset::Component(std::uint32_t index, CAssetComponent &component) const
{
    ...
    if (!ReadComponentRow(component_table_, static_cast<std::size_t>(index) * sizeof(DiskCAssetComponent),
                          disk_component))
    {
        return false;
    }
