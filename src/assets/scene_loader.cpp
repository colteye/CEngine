                                                ...
                                                Entities::EntityFactory &factory)
{
    ...
    asset_references.reserve(file.AssetReferences().size);
    const auto disk_asset_references = file.AssetReferences();
    for (std::size_t index = 0; index < disk_asset_references.size; ++index)
    {
        const auto &reference = disk_asset_references[index];
        ...
        if (!store.Resolve(file.String(reference.path_offset, reference.path_size), type, reference.guid, resolved))
        {
            return nullptr;
        }
        asset_references.push_back(std::move(resolved));
    }
    scene->SetAssetReferences(std::move(asset_references));

    std::vector<SerializedEntity> records(file.Entities().size);
    const auto class_blocks = file.ClassBlocks();
    for (std::size_t index = 0; index < class_blocks.size; ++index)
    {
        const auto &block = class_blocks[index];
        ...
        while (auxiliary_offset < auxiliary.size)
        {
            ...
            if (!ReadU32LE(auxiliary, auxiliary_offset, asset_index))
            {
            ...
            const AssetReference *reference = scene->AssetReference(asset_index);
        ...
        for (std::uint32_t row = 0; row < block.count; ++row)
        {
            ...
            records[entity_index] = {
                classname,           bytes.data + (static_cast<std::size_t>(row) * block.record_stride),
                block.record_stride, block.version,
                auxiliary_base,
            };
        }
    }
    ...
    for (std::size_t index = 0; index < file.Entities().size; ++index)
    {
        ...
        const std::string_view classname = file.String(row.classname_offset, row.classname_size);
        ...
        std::unique_ptr<Scene::Entity> entity =
            factory.Load(classname, serialized.version, serialized.bytes, serialized.size, serialized.auxiliary_base);
        if (entity == nullptr)
        {
            return nullptr;
        }
        entity->SetFlags(row.flags);
        entities.push_back(&scene->AddEntity(std::move(entity), file.String(row.name_offset, row.name_size)));
    }
    ...
    for (const auto &entity : scene->Entities())
    {
        ...
        if (entity->Classname() == "skybox" && dynamic_cast<const Entities::SkyboxEntity &>(*entity).enabled)
        {
            ++enabled_skyboxes;
        }
    ...
    if (enabled_skyboxes > 1 || enabled_fogs > 1 || enabled_post_processes > 1)
    {
    ...
    try
    {
        const auto connections = file.Connections();
