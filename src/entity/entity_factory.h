...
class EntityFactory {
public:
    using Loader = std::function<std::unique_ptr<Scene::Entity>(
        const std::uint8_t*, std::size_t, std::uint32_t)>;
    ...
    bool Register(std::string classname, Args&&... args)
    {
        ...
        return Register(std::move(classname), SerializedType::Version,
                ...
                std::uint32_t auxiliary_base) -> std::unique_ptr<Scene::Entity>
            {
                auto entity = std::apply(
                    [](const auto&... values) {
                        return std::make_unique<EntityType>(values...);
                    },
                    constructor_args);
    ...
    std::unique_ptr<Scene::Entity> Load(
...
private:
    ...
    static void ApplyTransform(
        const SerializedTransform& source, Scene::Transform& target)
    {
        target.position = {
            source.position[0], source.position[1], source.position[2]};
        target.rotation = {
            source.rotation[3], source.rotation[0],
