...
std::string_view LightEntity::Classname() const { return "light"; }

Renderer::Light LightEntity::RenderLight() const
{
    Renderer::Light light;
    if (type == LightType::Sun) light.type = Renderer::LightType::Directional;
    else if (type == LightType::Spot) light.type = Renderer::LightType::Spot;
    light.position = GetTransform().position;
    light.direction = glm::normalize(
        glm::vec3(GetTransform().world_matrix * glm::vec4(0, 0, -1, 0)));
...
void LightEntity::Update(EngineContext& context, float)
{
    if (context.rendering == nullptr || !renderer_light_) return;
