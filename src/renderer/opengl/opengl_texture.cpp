...
void ConfigureAnisotropicFiltering()
{
	...
	glTexParameterf(
		GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY, std::min(maximum, 8.0f));
}
...
GLuint UploadRgbe(const TextureMip& mip)
{
	...
	static const std::array<std::uint16_t, 256u * 256u> LinearHalf = [] {
		constexpr float MaxFiniteHalf = 65504.0f;
		std::array<std::uint16_t, 256u * 256u> values{};
		for (std::size_t exponent_byte = 0; exponent_byte < 256u;
			++exponent_byte)
		{
			...
			for (std::size_t mantissa = 0; mantissa < 256u; ++mantissa)
			{
				values[exponent_byte * 256u + mantissa] =
					glm::packHalf1x16(std::min(
						static_cast<float>(mantissa) * multiplier,
						MaxFiniteHalf));
			}
		}
		return values;
	...
	for (std::size_t index = 0; index < mip.data.size(); index += 4u)
	{
		const std::size_t row =
			static_cast<std::size_t>(mip.data[index + 3]) * 256u;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F,
		...
		0, GL_RGBA, GL_HALF_FLOAT, pixels.data());
	glGenerateMipmap(GL_TEXTURE_2D);
	glTexParameteri(
		GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, FullMipLevelCount(mip.width, mip.height));
	return texture;
}
...
GLuint OpenGLTexture::Load(const Texture& source)
{
	...
	{
		if (source.mips.size() != 1 || source.mips.front().data.size() !=
			static_cast<std::size_t>(source.mips.front().width) *
				source.mips.front().height * 4u)
		{
			Logging::Logger::Get().Error(
				"renderer", "RGBExp32 texture data is invalid");
			return 0;
		}
	...
	else
	{
		...
		for (std::size_t level = 0; level < source.mips.size(); ++level)
		{
			...
			glCompressedTexImage2D(GL_TEXTURE_2D, static_cast<GLint>(level),
				format, static_cast<GLsizei>(mip.width),
				static_cast<GLsizei>(mip.height), 0,
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL,
			source.mips.size() == 1
				? FullMipLevelCount(
					source.mips.front().width, source.mips.front().height)
...
GLuint OpenGLTexture::CreateSolid(
	GLubyte red, GLubyte green, GLubyte blue, GLubyte alpha)
{
	...
	glBindTexture(GL_TEXTURE_2D, texture);
	glTexImage2D(
		GL_TEXTURE_2D, 0, GL_RGBA8, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixel);
