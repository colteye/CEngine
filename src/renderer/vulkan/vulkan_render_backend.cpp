...
bool VulkanRenderBackend::Initialize(RenderSystem &, GLFWwindow *window, int window_width, int window_height)
{
    ...
    if (!CreateInstance() || !CreateSurface(window) || !PickPhysicalDevice() || !CreateLogicalDevice() ||
        !CreateSwapchain(window_width, window_height) || !CreateImageViews() || !CreateRenderPass() ||
...
bool VulkanRenderBackend::Resize(int window_width, int window_height)
{
    ...
    DestroySwapchainResources();
    if (!CreateSwapchain(window_width, window_height) || !CreateImageViews() || !CreateRenderPass() ||
    ...
    VkPhysicalDevice candidate) const
{
    ...
    if (present_mode_count != 0)
    {
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(candidate, surface, &present_mode_count,
                                                 ...
                                                 int window_height) const
{
    ...
    VkExtent2D actual_extent = {static_cast<uint32_t>(window_width), static_cast<uint32_t>(window_height)};
...
bool VulkanRenderBackend::RecordCommandBuffer(VkCommandBuffer command_buffer, uint32_t image_index)
{
    ...
    return VkSucceeded(vkEndCommandBuffer(command_buffer), "Failed to record Vulkan command buffer.");
}
