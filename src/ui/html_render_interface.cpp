...
/**
 * @file src/ui/html_render_interface.cpp
...
void HtmlRenderInterface::BeginFrame(int width, int height)
{
    frame_.Clear();
...
Renderer::UiFrame HtmlRenderInterface::TakeFrame()
{
    return std::move(frame_);
}

Rml::CompiledGeometryHandle HtmlRenderInterface::CompileGeometry(Rml::Span<const Rml::Vertex> vertices,
...
void HtmlRenderInterface::RenderGeometry(Rml::CompiledGeometryHandle handle, Rml::Vector2f translation,
                                          Rml::TextureHandle texture_handle)
{
    ...
    {
        Logging::Logger::Get().Error("ui", "HTML UI frame exceeded the 32-bit vertex range");
        return;
    }
    for (int index : geometry->indices)
    {
        ...
        {
            Logging::Logger::Get().Error("ui", "HTML UI emitted an invalid geometry index");
            return;
        }
    }
...
void HtmlRenderInterface::ReleaseGeometry(Rml::CompiledGeometryHandle handle)
{
    delete reinterpret_cast<Geometry *>(handle);
}

Rml::TextureHandle HtmlRenderInterface::LoadTexture(Rml::Vector2i &texture_dimensions, const Rml::String &source)
{
    texture_dimensions = {};
    Logging::Logger::Get().Warning("ui",
                                   "HTML image source is not a cooked CEngine texture and was rejected: " + source);
    return {};
}

Rml::TextureHandle HtmlRenderInterface::GenerateTexture(Rml::Span<const Rml::byte> source,
                                                         Rml::Vector2i source_dimensions)
{
    ...
    {
        Logging::Logger::Get().Error("ui", "HTML UI generated an invalid RGBA texture");
        return {};
    }
...
void HtmlRenderInterface::ReleaseTexture(Rml::TextureHandle handle)
{
    delete reinterpret_cast<Texture *>(handle);
}

void HtmlRenderInterface::EnableScissorRegion(bool enable)
{
    scissor_enabled_ = enable;
}

void HtmlRenderInterface::SetScissorRegion(Rml::Rectanglei region)
{
...
void HtmlRenderInterface::SetTransform(const Rml::Matrix4f *transform)
{
