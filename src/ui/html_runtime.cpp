...
#include "ui/html_render_interface.h"
...
class HtmlRuntime::Impl
{
  public:
    ...
    std::unique_ptr<HtmlFileInterface> file_interface;
    std::unique_ptr<HtmlSystemInterface> system_interface;
    std::unique_ptr<HtmlRenderInterface> render_interface;
...
HtmlRuntime::HtmlRuntime() : impl_(std::make_unique<Impl>())
{
}
...
bool HtmlRuntime::Initialize(Window::WindowSystem &window, std::filesystem::path content_root)
{
    Shutdown();
    if (RuntimeActive || !std::filesystem::is_directory(content_root))
    {
        Logging::Logger::Get().Error("ui", RuntimeActive ? "only one HTML UI runtime may be active"
                                                         : "UI content root does not exist");
        return false;
    }

    impl_->window = &window;
    impl_->file_interface = std::make_unique<HtmlFileInterface>(std::move(content_root));
    impl_->system_interface = std::make_unique<HtmlSystemInterface>(window);
    impl_->render_interface = std::make_unique<HtmlRenderInterface>();
    ...
    if (!Rml::Initialise())
    {
        Logging::Logger::Get().Error("ui", "failed to initialize the HTML UI runtime");
    ...
    {
        Logging::Logger::Get().Error("ui", "failed to create the HTML UI context");
        Rml::Shutdown();
        Rml::SetRenderInterface(nullptr);
        Rml::SetSystemInterface(nullptr);
        Rml::SetFileInterface(nullptr);
...
bool HtmlRuntime::Initialized() const
{
    return impl_->initialized;
}

bool HtmlRuntime::LoadFont(std::string_view path, bool fallback)
{
    return impl_->initialized && !path.empty() && Rml::LoadFontFace(Rml::String(path), fallback);
}

UiScreenHandle HtmlRuntime::LoadScreen(std::string_view path)
{
    if (!impl_->initialized || path.empty() ||
        std::filesystem::path(std::string(path)).extension() != ".html")
    {
        if (impl_->initialized && !path.empty())
        {
            Logging::Logger::Get().Error("ui", "UI screens must use the .html extension: " + std::string(path));
        }
        return {};
    }
...
bool HtmlRuntime::UnloadScreen(UiScreenHandle handle)
{
...
bool HtmlRuntime::Show(UiScreenHandle handle, bool modal)
{
    Impl::Screen *screen = impl_->Resolve(handle);
    if (screen == nullptr)
    {
...
bool HtmlRuntime::Hide(UiScreenHandle handle)
{
    Impl::Screen *screen = impl_->Resolve(handle);
    if (screen == nullptr)
    {
...
bool HtmlRuntime::BindClick(UiScreenHandle handle, std::string_view element_id, std::string action)
{
    Impl::Screen *screen = impl_->Resolve(handle);
...
bool HtmlRuntime::BindChange(UiScreenHandle handle, std::string_view element_id, std::string action)
{
    Impl::Screen *screen = impl_->Resolve(handle);
    if (screen == nullptr || element_id.empty() || action.empty())
    {
...
bool HtmlRuntime::SetText(UiScreenHandle handle, std::string_view element_id, std::string_view text)
{
    Impl::Screen *screen = impl_->Resolve(handle);
    Rml::Element *element =
...
bool HtmlRuntime::SetValue(UiScreenHandle handle, std::string_view element_id, float value)
{
    Impl::Screen *screen = impl_->Resolve(handle);
    Rml::Element *element =
        screen != nullptr ? screen->document->GetElementById(Rml::String(element_id)) : nullptr;
...
bool HtmlRuntime::SetChecked(UiScreenHandle handle, std::string_view element_id, bool checked)
{
    Impl::Screen *screen = impl_->Resolve(handle);
    Rml::Element *element =
        screen != nullptr ? screen->document->GetElementById(Rml::String(element_id)) : nullptr;
...
void HtmlRuntime::Update(const Input::InputSystem &input)
{
    if (!impl_->initialized)
    {
        return;
...
Renderer::UiFrame HtmlRuntime::Compose()
{
    if (!impl_->initialized)
    {
        return {};
...
std::vector<UiEvent> HtmlRuntime::DrainEvents()
{
