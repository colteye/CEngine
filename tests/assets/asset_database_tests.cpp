...
bool DeduplicationAndTypesWork()
{
    ...
    std::string error;
    if (!WriteBinaryAsset(root / path, desc, &error)) return false;
    AssetDatabase database(root);
    const AssetHandle first = database.Acquire(path, AssetType::Mesh, guid, &error);
    const AssetHandle second = database.Acquire("assets/compiled/props/./crate.cmesh", AssetType::Mesh, guid, &error);
    const bool result = Expect(first && second && first == second, "duplicate asset requests should share a handle") &&
        ...
        Expect(!database.Acquire(path, AssetType::Material, guid, &error), "type mismatch should fail") &&
...
bool ReleaseInvalidatesHandles()
{
    ...
    AssetDatabase database(root);
    std::string error;
    const AssetHandle old = database.Acquire(path, AssetType::Texture, guid, &error);
    database.ReleaseAll();
    const AssetHandle replacement = database.Acquire(path, AssetType::Texture, guid, &error);
    const bool result = Expect(old && replacement, "texture references should resolve") &&
        ...
        Expect(!database.Acquire("../outside.cmesh", AssetType::Mesh, guid, &error),
...
int main() { return DeduplicationAndTypesWork() && ReleaseInvalidatesHandles() ? 0 : 1; }
