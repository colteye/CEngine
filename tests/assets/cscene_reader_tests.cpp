...
std::vector<std::uint8_t> MinimalPayload()
{
    ...
    header.entity_stride = sizeof(DiskSceneEntity);
    header.class_table_offset = header.entity_table_offset + sizeof(DiskSceneEntity);
    header.class_count = 1;
    header.class_stride = sizeof(DiskEntityClassBlock);
    ...
    entity.name_offset = 5;
    entity.name_size = 4;
    DiskEntityClassBlock block;
    block.classname_size = 5;
    block.flags = EntityClassBlockRequired;
    block.count = 1;
    block.record_stride = sizeof(DiskTransform);
    block.entity_indices_offset = indices_offset;
    block.records_offset = transforms_offset;
    DiskTransform transform;
    std::vector<std::uint8_t> payload;
    Append(payload, header);
    Append(payload, settings);
    Append(payload, entity);
    Append(payload, block);
    Append(payload, std::uint32_t{0});
    Append(payload, transform);
...
bool ValidFixtureLoads()
{
    ...
    const auto path = root / "minimal.cscene";
    if (!Write(path, MinimalPayload()))
    {
        return false;
    }
    CSceneFile file;
    const bool result =
        Expect(file.Load(path), "fixture should load") &&
        ...
        Expect(file.String(file.Entities()[0].classname_offset, file.Entities()[0].classname_size) == "empty",
               "classname should load") &&
...
bool InvalidRangesFail()
{
    ...
    const auto bad_offset = root / "bad-offset.cscene";
    Write(bad_offset, payload);
    if (!Expect(!file.Load(bad_offset), "overflowing table offset should fail"))
    {
        return false;
    }
    ...
    block->record_stride = UINT32_MAX;
    block->count = UINT32_MAX;
    const auto bad_size = root / "bad-size.cscene";
    Write(bad_size, payload);
    if (!Expect(!file.Load(bad_size), "overflowing class range should fail"))
    {
        return false;
    }
    payload = MinimalPayload();
    header = reinterpret_cast<DiskSceneHeader *>(payload.data());
    header->version = 99;
    const auto bad_version = root / "bad-version.cscene";
    Write(bad_version, payload);
    const bool result = Expect(!file.Load(bad_version), "unsupported scene version should fail");
    std::filesystem::remove_all(root);
    return result;
}
} // namespace
...
int main(int argc, char **argv)
{
    if (argc == 2)
    {
        ...
        return Expect(file.Load(argv[1]), "Python fixture should load") &&
                       Expect(file.Entities().size == 6, "Python fixture should contain six entities") &&
                       Expect(file.ClassBlocks().size == 4, "Python fixture should contain four class blocks") &&
                       Expect(file.Connections().size == 1, "Python fixture should contain one connection") &&
                       Expect(file.String(file.Entities()[0].classname_offset, file.Entities()[0].classname_size) ==
                                  "light",
                              "Python fixture classname should match")
                   ? 0
                   : 1;
    }
