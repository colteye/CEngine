...
bool Contains(const DirectionalShadowCascade& cascade, const std::array<glm::vec3, 8>& corners) {
    ...
    for (const glm::vec3& corner : corners) {
        ...
        if (ndc.x < -1.0f - epsilon || ndc.x > 1.0f + epsilon || ndc.y < -1.0f - epsilon ||
            ndc.y > 1.0f + epsilon || ndc.z < -1.0f - epsilon || ndc.z > 1.0f + epsilon) {
            return false;
        }
    }
    return true;
...
bool TestDistantOverlappingCasterIsIncluded() {
    ...
    const DirectionalShadowCascade cascade = BuildDirectionalShadowCascade(
        receiver, glm::vec3(0.0f, 0.0f, -1.0f), 1024, {caster}, 200.0f);
    return Expect(Contains(cascade, receiver),
                  "cascade must contain its complete receiver slice") &&
...
bool TestCasterOutsideFootprintDoesNotExpandDepth() {
    ...
    const Bounds outside =
        MakeBounds(glm::vec3(100.0f, 100.0f, 49.0f), glm::vec3(102.0f, 102.0f, 51.0f));
    const DirectionalShadowCascade with_outside_caster = BuildDirectionalShadowCascade(
        receiver, glm::vec3(0.0f, 0.0f, -1.0f), 1024, {outside}, 200.0f);
...
bool TestCasterRemainsIncludedAsReceiverMovesInDepth() {
    ...
    for (float receiver_z : {0.0f, -20.0f, -40.0f}) {
        const auto receiver = BoxCorners(glm::vec3(-5.0f, -5.0f, receiver_z - 5.0f),
                                         glm::vec3(5.0f, 5.0f, receiver_z + 5.0f));
        const DirectionalShadowCascade cascade = BuildDirectionalShadowCascade(
            receiver, glm::vec3(0.0f, 0.0f, -1.0f), 1024, {caster}, 200.0f);
        if (!Expect(Contains(cascade, receiver),
                    "moving receiver slice must remain inside its cascade") ||
            !Expect(Contains(cascade, BoxCorners(caster.min, caster.max)),
...
bool TestObliqueLightFacingCasterIsIncluded() {
    ...
    const Bounds caster =
        MakeBounds(caster_center - glm::vec3(1.0f), caster_center + glm::vec3(1.0f));
    const DirectionalShadowCascade cascade =
        BuildDirectionalShadowCascade(receiver, light_direction, 1024, {caster}, 200.0f);
    return Expect(Contains(cascade, receiver),
                  "oblique cascade must contain its complete receiver slice") &&
           Expect(Contains(cascade, BoxCorners(caster.min, caster.max)),
