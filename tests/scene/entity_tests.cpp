...
public:
    std::string_view Classname() const override { return "game_custom"; }
    void Update(CEngine::EngineContext&, float delta_seconds) override
...
public:
    void BeginFrame() override { ++frames; }
    bool IsDown(CEngine::Input::Key key) const override
    { return key == CEngine::Input::Key::W || key == CEngine::Input::Key::LeftShift; }
    glm::vec2 PointerDelta() const override { return {4.0f, -2.0f}; }
    void SetPointerCaptured(bool value) override { pointer_captured = value; }
...
bool GenerationalSlotsRejectStaleHandles()
{
    Scene scene;
    auto& first =
        scene.CreateEntity<CEngine::Entities::PropEntity>("Crate");
    ...
    auto& second =
        scene.CreateEntity<CEngine::Entities::LightEntity>("Light");
...
bool EntityClassesOwnTheirFields()
{
    Scene scene;
    auto& prop =
        scene.CreateEntity<CEngine::Entities::PropEntity>("HallwayCrate");
    prop.visible = false;
    prop.shadow_only = true;
    prop.GetTransform().position = {2.0f, 3.0f, 4.0f};
    prop.GetTransform().UpdateWorldMatrix();
    auto& light =
        scene.CreateEntity<CEngine::Entities::LightEntity>("KeyLight");
...
bool EnvironmentEntitiesOwnTheirFields()
{
    Scene scene;
    auto& sky =
        scene.CreateEntity<CEngine::Entities::SkyboxEntity>("Environment");
    sky.intensity = 1.5f;
    auto& fog =
        scene.CreateEntity<CEngine::Entities::FogEntity>("Atmosphere");
    fog.density = 0.04f;
    fog.GetTransform().position.z = 2.0f;
    auto& post_process =
        scene.CreateEntity<CEngine::Entities::PostProcessEntity>("SceneLook");
    ...
    return Expect(sky.Classname() == "skybox" && Near(sky.intensity, 1.5f),
        ...
        Expect(post_process.Classname() == "post_process" &&
                Near(post_process.exposure, 1.25f) &&
...
bool LightRendererRecordFollowsEntityLifetime()
{
    ...
    light.mode = CEngine::Entities::LightMode::Baked;
    light.Initialize(context);
    if (!Expect(rendering.GetDirectLights().size() == 1,
            "light initialization should register one renderer light") ||
        !Expect(!rendering.GetDirectLights()[0].enabled,
            "a baked light should keep a disabled renderer record"))
        return false;

    light.mode = CEngine::Entities::LightMode::Realtime;
    light.Update(context, 0.0f);
    if (!Expect(rendering.GetDirectLights().size() == 1 &&
            rendering.GetDirectLights()[0].enabled,
    ...
    const auto replacement =
        rendering.RegisterLight(CEngine::Renderer::Light{});
...
bool GameLayerCanAddEntitiesAndDriveLifecycle()
{
    ...
    fog.density = 0.125f;
    auto& post_process =
        scene.CreateEntity<CEngine::Entities::PostProcessEntity>("");
    ...
    return Expect(Near(custom.elapsed, 0.25f),
            "custom entity should receive the engine lifecycle") &&
...
bool InputSystemOwnsPlatformBackend()
{
    ...
    input.Set(move, -1.0f);
    input.BeginFrame();
    return Expect(backend_ptr->frames == 1,
            "input system should begin its platform backend frame") &&
        Expect(Near(input.Value(move), 1.0f),
            "registered key binding should produce its logical action") &&
        Expect(Near(input.Value(look), 2.0f),
            "registered pointer binding should scale its logical action") &&
        Expect(input.IsDown(CEngine::Input::Key::W) &&
                input.PointerDelta() == glm::vec2(4.0f, -2.0f),
...
int main(int argc, char** argv)
{
    if (argc == 2)
    {
        ...
        return Expect(scene != nullptr, "scene should load") &&
            Expect(!scene->Active(), "loaded scene should not publish entity lifecycle before bindings exist") &&
            Expect(scene->EntityCount() == 4, "loaded scene should preserve its authored entities") &&
            Expect(scene->Entities()[0]->Classname() == "light",
                "loaded entity should preserve its light class") &&
            ...
            Expect(Near(static_cast<CEngine::Entities::PostProcessEntity*>(
                    scene->Entities()[3].get())->exposure, 1.25f),
                "loaded post-process entity should preserve its scene settings") ? 0 : 1;
    }
    return GenerationalSlotsRejectStaleHandles() && EntityClassesOwnTheirFields() &&
        EnvironmentEntitiesOwnTheirFields() && SlotLookupWorks() &&
        LightModesHaveExplicitRuntimeDirectSemantics() &&
        LightRendererRecordFollowsEntityLifetime() &&
        GameLayerCanAddEntitiesAndDriveLifecycle() &&
        InputSystemOwnsPlatformBackend() ? 0 : 1;
}
