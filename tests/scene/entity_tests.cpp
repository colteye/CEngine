...
{
    Scene scene;
    auto& first = static_cast<CEngine::Entities::StaticProp&>(
        scene.CreateEntity("prop_static", "Crate"));
...
{
    Scene scene;
    auto& prop = static_cast<CEngine::Entities::StaticProp&>(
        scene.CreateEntity("prop_static", "HallwayCrate"));
    prop.visible = false;
    ...
    return Expect(prop.Classname() == "prop_static", "static prop classname should be fixed") &&
        Expect(prop.Name() == "HallwayCrate", "entity should own its name") &&
        Expect(!prop.visible, "static prop should own visibility field") &&
...
int main(int argc, char** argv)
{
    if (argc == 2)
    {
        ...
        return Expect(scene != nullptr, error.c_str()) &&
            Expect(scene->Active(), "loaded scene should be active") &&
            Expect(scene->EntityCount() == 2, "loaded scene should contain two entities") &&
            ...
            Expect(Near(scene->Entities()[0]->GetTransform().position.x, 1.0f), "loaded transform should match") ? 0 : 1;
    }
