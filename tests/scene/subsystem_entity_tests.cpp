    ...
    const std::filesystem::path& project_root)
{
    ...
    std::unique_ptr<CEngine::Scene::Scene> scene =
        CEngine::Assets::LoadScene(scene_path, assets, factory);
    if (!Expect(scene != nullptr, "scene should load")) return false;
    ...
    initial_ambient.enabled = glm::any(glm::greaterThan(
        scene->Settings().ambient_color, glm::vec3(0.0f)));
    ...
    for (const auto& entity : scene->Entities())
    {
        ...
        if (entity->Classname() == "prop")
        {
            ...
            if (prop.Enabled() && prop.visible) ++visible_prop_count;
            if (prop.shadow_only)
            {
    ...
    std::size_t active_renderable_count = 0;
    bool shadow_only_flags_are_valid = true;
    for (const auto& renderable : renderer.GetRenderables())
    {
        if (renderable.mesh != nullptr) ++active_renderable_count;
    ...
    return Expect(scene->EntityCount() > 0, "cooked scene should contain entities") &&
        ...
        Expect(active_renderable_count == visible_prop_count,
        ...
        Expect(found_skybox && ibl.enabled && !ambient.enabled &&
                ibl.panorama != nullptr && !ibl.panorama->Empty(),
...
int main(int argc, char** argv)
{
    ...
    const auto renderable_handle = renderer.RegisterRenderable(renderable);
    renderer.RemoveRenderable(renderable_handle);
    const auto current_renderable = renderer.RegisterRenderable(renderable);
    const std::uint64_t stable_renderable_revision =
        renderer.GetRenderableRevision();
    renderer.UpdateRenderable(
        current_renderable, glm::mat4(1.0f), renderable.flags);
    const bool unchanged_renderable_is_free =
        renderer.GetRenderableRevision() == stable_renderable_revision;
    renderer.UpdateRenderable(current_renderable,
        glm::translate(glm::mat4(1.0f), glm::vec3(1.0f)),
        renderable.flags);
    const bool changed_renderable_is_published =
        renderer.GetRenderableRevision() == stable_renderable_revision + 1;
    ...
    const std::uint64_t stable_light_revision =
        renderer.GetLightStateRevision();
    renderer.UpdateLight(current_light, light);
    const bool unchanged_light_is_free =
        renderer.GetLightStateRevision() == stable_light_revision;
    light.intensity = 2.0f;
    renderer.UpdateLight(current_light, light);
    const bool changed_light_is_published =
        renderer.GetLightStateRevision() == stable_light_revision + 1;
    ...
    const bool result =
        ...
        Expect(renderer.ResolveRenderable(renderable_handle) == nullptr,
            "stale renderable handle should not resolve") &&
        Expect(renderer.ResolveRenderable(current_renderable) != nullptr,
            "current renderable handle should resolve") &&
        Expect(unchanged_renderable_is_free,
            "an unchanged renderable update should not dirty renderer state") &&
        Expect(changed_renderable_is_published,
            "a changed renderable should advance renderer state once") &&
        ...
        Expect(renderer.ResolveLight(light_handle) == nullptr,
            "stale light handle should not resolve") &&
        Expect(renderer.ResolveLight(current_light) != nullptr,
            "current light handle should resolve") &&
        Expect(unchanged_light_is_free,
            "an unchanged light update should not dirty renderer state") &&
        Expect(changed_light_is_published,
            "a changed light should advance renderer state once") &&
        Expect(other_renderer.GetRenderables().empty() &&
                other_renderer.GetDirectLights().empty(),
    ...
    auto& prop =
        scene.CreateEntity<CEngine::Entities::PropEntity>("MovingCrate");
    prop.dynamic = true;
    prop.collision_enabled = true;
    prop.collision_half_extents = {1.0f, 2.0f, 3.0f};
    prop.mass = 8.0f;
    auto& static_prop =
        scene.CreateEntity<CEngine::Entities::PropEntity>("Floor");
